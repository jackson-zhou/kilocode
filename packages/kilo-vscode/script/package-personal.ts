...
export const PERSONAL = {
  ...
  publisher: "jackson",
  displayName: "Kilo Code Personal: Agent Review Build",
...
export function metadata(source: string) {
  ...
  const text = runtime(JSON.stringify(data)).replaceAll("Kilo Code", "Kilo Code Personal")
    ...
    .then(async () => {
      ...
      await Promise.all(
        dist
          ...
          .map((file) => fs.readFile(file, "utf8").then((text) => fs.writeFile(file, runtime(text)))),
      )
